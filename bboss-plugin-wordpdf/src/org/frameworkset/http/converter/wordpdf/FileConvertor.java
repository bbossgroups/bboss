...
//				e.printStackTrace();
	...
	public static  File realWordConvertor(String flashpaperWorkDir, String wordtemplate,
			...
			if (bookMarks != null && bookMarks.length > 0) {
				...
				for (String key : bookMarks) {
					...
					if (bookMarkExist) {
						...
						Dispatch range = Dispatch.call(rangeItem, "Range")
								.toDispatch();
						Dispatch.put(range, "Text", new Variant(bookdatas[i]));// 插入书签的值
					}
					i++;
				}
			}
			...
			if (bookdatas != null && bookdatas.size() > 0) {
				...
				for (String key : keys) {
					...
					if (bookMarkExist) {
						...
						Dispatch range = Dispatch.call(rangeItem, "Range")
								.toDispatch();
						Dispatch.put(range, "Text", new Variant(bookdatas.get(key)));// 插入书签的值
					}
