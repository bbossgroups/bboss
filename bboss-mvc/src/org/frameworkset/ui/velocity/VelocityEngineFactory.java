 ...
 * {@link org.frameworkset.web.servlet.view.velocity.VelocityConfigurer}
 * for web views.
 *
 ...
 * resource loader path via Bboss's Resource abstraction, possibly relative
 * to the Bboss application context.
 *
 * <p>If "overrideLogging" is true (the default), the VelocityEngine will be
 * configured to log via Commons Logging, that is, using the Bboss-provided
...
public class VelocityEngineFactory {
	...
	protected final static Logger logger = Logger.getLogger(VelocityEngineFactory.class);
	 ...
	 * Bboss resource location via the "resourceLoaderPath" property.
	...
	/**
	 * Set the Velocity resource loader path via a Bboss resource location.
	 ...
	 * a generic BbossResourceLoader will be used under the name "Bboss", without
	 ...
	 * detect changes. With BbossResourceLoader, the resource will be cached
	 ...
	 * <p>To enforce the use of BbossResourceLoader, i.e. to not resolve a path
	 * as file system resource in any case, turn off the "preferFileSystemAccess"
	 * flag. See the latter's javadoc for details.
	 * @see #setResourceLoader
	 * @see #setVelocityProperties
	 * @see #setPreferFileSystemAccess
	 * @see BbossResourceLoader
	...
	/**
	 * Set the Bboss ResourceLoader to use for loading Velocity template files.
	...
	/**
	 * Return the Bboss ResourceLoader to use for loading Velocity template files.
	 */
	 ...
	 * <p>Default is "true". Turn this off to always load via BbossResourceLoader
	 ...
	 * either a standard Velocity FileResourceLoader or a BbossResourceLoader.
	 * <p>Called by <code>createVelocityEngine()</code>.
	 * @param velocityEngine the VelocityEngine to configure
	 * @param resourceLoaderPath the path to load Velocity resources from
	 * @see org.apache.velocity.runtime.resource.loader.FileResourceLoader
	 * @see BbossResourceLoader
	 * @see #initBbossResourceLoader
	 * @see #createVelocityEngine()
	 */
	protected void initVelocityResourceLoader(VelocityEngine velocityEngine, String resourceLoaderPath) {
		if (isPreferFileSystemAccess()) {
			// Try to load via the file system, fall back to BbossResourceLoader
			// (for hot detection of template changes, if possible).
			try {
				StringBuilder resolvedPath = new StringBuilder();
				String[] paths = StringUtil.commaDelimitedListToStringArray(resourceLoaderPath);
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Cannot resolve resource loader path [" + resourceLoaderPath +
							"] to [java.io.File]: using BbossResourceLoader", ex);
				}
				initBbossResourceLoader(velocityEngine, resourceLoaderPath);
			}
		}
		else {
			// Always load via BbossResourceLoader
			// (without hot detection of template changes).
			if (logger.isDebugEnabled()) {
				logger.debug("File system access not preferred: using BbossResourceLoader");
			}
			initBbossResourceLoader(velocityEngine, resourceLoaderPath);
		}
	}
	...
	/**
	 * Initialize a BbossResourceLoader for the given VelocityEngine.
	 ...
	 */
	protected void initBbossResourceLoader(VelocityEngine velocityEngine, String resourceLoaderPath) {
		velocityEngine.setProperty(
				RuntimeConstants.RESOURCE_LOADER, VelocityResourceLoader.NAME);
		velocityEngine.setProperty(
				VelocityResourceLoader.RESOURCE_LOADER_CLASS, VelocityResourceLoader.class.getName());
		velocityEngine.setProperty(
				VelocityResourceLoader.RESOURCE_LOADER_CACHE, "true");
		velocityEngine.setApplicationAttribute(
				VelocityResourceLoader.RESOURCE_LOADER, getResourceLoader());
		velocityEngine.setApplicationAttribute(
				VelocityResourceLoader.RESOURCE_LOADER_PATH, resourceLoaderPath);
	}
