...
var DataGrid = function(id, _params) {
	...
	this.loadData = function(params) {
		...
		DataGrid.log("loadData().params1="+jQuery.param(params));
	...
	try {
		if (params instanceof jQuery) {
			...
			$.each(params, function(i, o){
				//eval函数不安全，同时在ff浏览器上也无法运行，可以考虑用其他方法取代				
				window.eval("queryParams."+o.name+"=\""+o.value+"\"");
				//queryParams.unitcost = "4";
			});
		} else {
			DataGrid.log("DataGrid.addQueryParams().params is not a jQuery Object");
			$.each(params, function(n, v){
				window.eval("queryParams."+n+"=\""+v+"\"");
			});
		}
	} catch(e) {
