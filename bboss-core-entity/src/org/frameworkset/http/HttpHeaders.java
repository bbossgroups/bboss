...
public class HttpHeaders implements MultiValueMap<String, String>, Serializable {
	...
	private Boolean jsonRequest = null;
	...
	public List<MediaType> getAccept() {
		...
		if (result.size() == 1) {
			...
			if (acceptHeader.size() > 1) {
				value = BaseSimpleStringUtil.collectionToCommaDelimitedString(acceptHeader);
	...
	public void setAccessControlAllowMethods(List<HttpMethod> allowedMethods) {
		set(ACCESS_CONTROL_ALLOW_METHODS, BaseSimpleStringUtil.collectionToCommaDelimitedString(allowedMethods));
	}
	...
	public void setAllow(Set<HttpMethod> allowedMethods) {
		set(ALLOW, BaseSimpleStringUtil.collectionToCommaDelimitedString(allowedMethods));
	}
	...
	public Set<HttpMethod> getAllow() {
		...
		if (!BaseSimpleStringUtil.isEmpty(value)) {
	...
	public MediaType getContentType() {
		...
		return (BaseSimpleStringUtil.hasLength(value) ? MediaType.parseMediaType(value) : null);
	}

	/**
	 * Return the json type of the body, as specified
	 * by the {@code Content-Type} header.
	 * <p>Returns {@code null} when the content-type is unknown.
	 */
	public boolean isJsonRequest() {
		...
		String value = getFirst(CONTENT_TYPE);
		if(value != null){
			jsonRequest = value.startsWith(MediaType.APPLICATION_JSON_VALUE) || value.startsWith(MediaType.APPLICATION_JSONP_VALUE);
		}
		else{
			jsonRequest = false;
		}
