 ...
 * @version 1.0
 */
public class UnicodeStream_int_InputStream_intTypeMethod extends BaseTypeMethod{
	@Override
	public void action(StatementInfo stmtInfo, Param param, PreparedStatement statement, PreparedStatement statement_count, List resources) throws SQLException {
		...
		try
		{
			...
			statement.setUnicodeStream(param.getIndex(), in,((Integer)data[1]).intValue());
			if(statement_count != null)
			{
				statement_count.setUnicodeStream(param.getIndex(), in,((Integer)data[1]).intValue());
			}
		}
		finally
		{
