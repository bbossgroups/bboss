...
//	protected boolean outcon = true;
	...
	private void setUpParams(StatementInfo stmtInfo,Params Params,PreparedStatement statement,PreparedStatement statement_count,List resources)throws SQLException
	{
		...
		for(int i = 0; i < Params.params.size(); i ++)
		{
			...
			param.method.action(stmtInfo,param,statement,statement_count,resources);
	...
	public <T> T executePreparedForObject(DBOptions dbOptions,Class<T> objectType) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
		return executePreparedForObject(  dbOptions,null,objectType);
	}

	public <T> T[] executePreparedForObjectArray(DBOptions dbOptions,Class<T> objectType) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
		return executePreparedForObjectArray(  dbOptions,null,objectType);
	}

	public <T> List<T> executePreparedForList(DBOptions dbOptions,Class<T> objectType) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
	...
	public <T> T executePreparedForObject(DBOptions dbOptions,Connection con,Class<T> objectType) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
		return (T)executePreparedForObject(  dbOptions,con,objectType,null);
	}

	public <T> T[] executePreparedForObjectArray(DBOptions dbOptions,Connection con,Class<T> objectType) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
		return (T[])executePreparedForObjectArray(  dbOptions,con,objectType,null);
	}

	public <T> List<T> executePreparedForList(DBOptions dbOptions,Connection con,Class<T> objectType) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
	 ...
	 * 带行处理器的方法开始
	 *****************************************************************/
	public <T> T executePreparedForObject(DBOptions dbOptions,Class<T> objectType,RowHandler rowhandler) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
		return (T)executePreparedForObject(  dbOptions,null,objectType,rowhandler);
	}

	public <T> T[] executePreparedForObjectArray(DBOptions dbOptions,Class<T> objectType,RowHandler rowhandler) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
		return (T[])executePreparedForObjectArray(  dbOptions,null,objectType,rowhandler);
	}

	public <T> List<T> executePreparedForList(DBOptions dbOptions,Class<T> objectType,RowHandler rowhandler) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
	...
	public <T> T executePreparedForObject(DBOptions dbOptions,Connection con,Class<T> objectType,RowHandler rowhandler) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
		if(objectType != null)
		{
	...
	public  <T> T[] executePreparedForObjectArray(DBOptions dbOptions,Connection con,Class<T> objectType,RowHandler rowhandler) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
		return (T[])innerExecute(  dbOptions,con,objectType,rowhandler,ResultMap.type_objectarray,false);
	}

	public <T> List<T> executePreparedForList(DBOptions dbOptions,Connection con,Class<T> objectType,RowHandler rowhandler) throws SQLException {
		if(this.batchparams != null && batchparams.size() > 0)
			throw new SQLException("Can not execute batch prepared operations as single prepared operation,Please call method executePreparedBatch()!");
		return (List<T>)innerExecute(  dbOptions,con,objectType,rowhandler,ResultMap.type_list,false);
	}
